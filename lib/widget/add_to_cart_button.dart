...
class AddToCartButtonState extends State<AddToCartButton> {
  ...
  Future<void> _loadCartState() async {
    _prefs = await SharedPreferences.getInstance();
    setState(() {
      _isClicked = _prefs.getBool('isClicked${widget.productName}${widget.productUnit}') ?? false;
      _count = _prefs.getInt('count${widget.productName}${widget.productUnit}') ?? 0;
    });
  }

  Future<void> _saveCartState() async {
    await _prefs.setBool('isClicked${widget.productName}${widget.productUnit}', _isClicked);
    await _prefs.setInt('count${widget.productName}${widget.productUnit}', _count);
  }
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        return ListView.builder(
          itemCount: widget.productItems?.length ?? 0,
          itemBuilder: (context, index) {
            final item = widget.productItems![index];
            return ListTile(
              title: Text('${widget.productName} - ${item.unit}'),
